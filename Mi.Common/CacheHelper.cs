...
namespace Mi.Common
{
    ...
    public class CacheHelper
    {
        ...
        /// <param name="key">缓存Key</param>
        /// <param name="value">缓存Value</param>
        /// <param name="expiresMinutes">缓存时长(分钟)</param>
        /// <param name="isSliding">是否滑动过期（如果在过期时间内有操作，则以当前时间点延长过期时间）</param>
        /// <returns></returns>
        public static bool Set(string key, object value, int expiresMinutes, bool isSliding = false)
        {
            return Set(key, value, TimeSpan.FromMinutes(expiresMinutes), isSliding);
        }
        ...
        {
            return Set(key, value, TimeSpan.FromMinutes(24*60), true);
        }
